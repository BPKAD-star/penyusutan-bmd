...
// 2026-09-23) — dua mapper, kembar dgn Pengadaan.tsx.
function barangDariDraft(it: DraftItem): BarangTransaksi {
  const f = it.fields || {}
  return {
    ...
    tglPerolehan: it.tglPerolehan || null, jumlah: 1, satuan: it.satuan || null, nilai: toNum(it.harga),
  }
}
// ⚠️ `l.tgl_perolehan`, BUKAN `l.tanggal` — kolom "Tgl Perolehan" di tabel
// approved SEBELUM ini salah membaca `l.tanggal` (tanggal BAST/ledger),
// bukan tanggal barang itu DIBUAT. Bug tampilan murni (bukan ledger — baris
// ledgernya sendiri sudah benar), ketahuan & ditutup sekalian saat menyusun
// mapper ini; lihat komentar `JurnalLine.tgl_perolehan` di atas & peringatan
// baris ~574 ("WAJIB l.tgl_perolehan, BUKAN l.tanggal") yang sudah lama ada
// utk jalur Buka Kunci — kolom tampilan ini kelewat.
function barangDariLine(l: JurnalLine): BarangTransaksi {
  ...
  return {
    ...
    noPolisi: f.no_polisi || null, noMesin: f.no_mesin || null, noRangka: f.no_rangka || null,
    luas: f.luas || null, alamatDetail: f.alamat_detail || null,
    tglPerolehan: l.tgl_perolehan || null, jumlah: 1, satuan: l.satuan, nilai: l.nilai,
  }
}
...
export default function PerolehanManual({ kategori, judul, pihakLabel }: {
  ...
  // menyentuh ledger, cuma UPDATE payload jurnal_header, jadi tak melanggar
  // append-only. Kembar dgn Pengadaan.tsx — ubah satu, samakan yang lain.
  async function hapusDraftItems(h: Jurnal, keys: string[]) {
    if (keys.length === 0) return
    // Satu barang → dialognya menyebut NAMANYA (itu yang dulu diberikan dialog
    // per-baris), dan peringatan "centang bertahan lintas pencarian" ikut
    // dilepas karena memang tak berlaku untuk satu barang.
    const it = keys.length === 1 ? (h.payload.draft_items || []).find(i => i.key === keys[0]) : undefined
    if (!(await konfirmasi({
      nada: 'merah', ikon: '🗑',
      judul: it ? 'Hapus barang ini dari draft?' : 'Hapus barang yang dicentang?',
      subjudul: it ? (it.fields?.nama_barang || it.kode || `Dokumen ${h.no_sk}`) : `Dokumen ${h.no_sk}`,
  ...
  async function hapusDokumen(h: Jurnal) {
    if (h.hasLedger) {
      ...
      if (error) { await konfirmasiGagal(konfirmasi, `Gagal mengarsipkan dokumen: ${error.message}`); return }
    ...
    if (error) { await konfirmasiGagal(konfirmasi, `Gagal menghapus dokumen: ${error.message}`); return }
  ...
  // pola & alasan kembar dgn Pengadaan.tsx: strip merah di ATAS halaman luput
  // kalau kartunya di tengah daftar panjang; pop-up muncul TEPAT di tempat
  // tombol Setujui ditekan, baik gagal DIVALIDASI maupun gagal DIEKSEKUSI.
  ...
  async function approveHeader(h: Jurnal) {
    ...
    if (items.length === 0) { await gagalSetujui('Dokumen ini belum ada barangnya — tambahkan dulu sebelum disetujui.'); return }
    ...
    if (!h.payload.dokumen_paths || h.payload.dokumen_paths.length === 0) {
      await gagalSetujui('Dokumen BAST belum diunggah — lengkapi dulu (✎ Edit Dokumen) sebelum dokumen ini disetujui.')
      return
    }
    for (const it of items) {
      if (!it.kode) { await gagalSetujui('Ada barang draft tanpa kode.'); return }
      if (!it.tglPerolehan) { await gagalSetujui(`Barang "${it.fields.nama_barang || it.kode}" belum ada tanggal perolehan.`); return }
      if (toNum(it.harga) <= 0) { await gagalSetujui(`Nilai "${it.fields.nama_barang || it.kode}" harus > 0.`); return }
      ...
      if (!it.foto || it.foto.length === 0) {
        await gagalSetujui(`Barang "${it.fields.nama_barang || it.kode}" belum ada foto — lengkapi dulu (✎ Edit Spesifikasi) sebelum dokumen ini disetujui.`)
        return
      }
    }
    ...
    // lihat catatan kembar di Pengadaan.tsx. Gagal diam-diam di sini membuat
    // SELURUH barang satu dokumen tercatat intrakomptabel tanpa satu pun error.
    let batasMap: Map<string, number | null>
    try {
      batasMap = await fetchBatasKapitalisasi(supabase, items.map(it => it.kode))
    } catch (e) {
      await gagalSetujui((e as Error).message); setBusyId(null); return
    }
    ...
    if (asetErr || !inserted) { await gagalSetujui(`Gagal membuat barang: ${asetErr?.message}`); setBusyId(null); return }
  ...
  async function unapproveHeader(j: Jurnal) {
    ...
    for (const l of j.lines) {
      ...
      if (error) { setBusyId(null); await konfirmasiGagal(konfirmasi, String(error)); return }
    }
...
function PendingCard({ h, isAdmin, busy, golonganLabels, pihakLabel, onEditHeader, onHapusDokumen, onTambah, onHapusItems, onEditSpes, onApprove }: {
        ...
        <>
          ...
          <div className="overflow-x-auto">
            {/* `table-fixed` + colgroup TETAP (lib/kolomBarangTransaksi.ts,
                keputusan user 2026-09-23) — lihat catatan panjang di
                Pengadaan.tsx (`PendingCard`); kolom ekstra di sini cuma
                Foto & Keterangan (Komptabel baru tampil sesudah approve). */}
            <table className="w-full table-fixed">
              <ColgroupBarang
                sebelum={[{ key: 'chk', berat: 3 }]}
                sesudah={[{ key: 'foto', berat: 5 }, { key: 'keterangan', berat: 10 }]} />
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  {/* Kolom ikon 🗑 per baris DICABUT — kembar dgn kartu draft
                      Pengadaan.tsx; alasan lengkapnya di sana (satu pintu
                      hapus/edit lewat centang, & tombol merah yang dulu duduk
                      persis di sebelah kotak centang). */}
                  <KolomBarangHead />
              ...
              <tbody className="divide-y divide-gray-50">
                ...
                {sel.terlihat.length === 0 && (
                  <tr><td colSpan={15} className="table-td text-center text-xs text-gray-400 py-6">Tak ada barang yang cocok dengan pencarian.</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function DraftRow({ item, checked, onToggle, fotoUrl }: {
  ...
  return (
    <tr>
      ...
      {/* ⚠️ Tanggal Perolehan kosong tampil '-' generik (KolomBarangCells),
          BUKAN lagi tanda ⚠ khusus spt sebelumnya — kolom kanonik dipakai
          sama di sembilan menu & tak bisa tahu menu mana yang mewajibkannya.
          Validasi sesungguhnya TETAP ada di `approveHeader` (`gagalSetujui`
          menolak barang tanpa tanggal perolehan sebelum Setujui), jadi
          operator tetap tak bisa lolos menyimpan tanpa mengisinya — yang
          hilang cuma peringatan dini di tabel, bukan penjaganya. */}
      <KolomBarangCells barang={barangDariDraft(item)} />
      <td className="table-td text-center">
        <FotoSel paths={item.foto} thumbUrl={fotoUrl} judul={item.fields?.nama_barang || item.uraianBarang} />
      </td>
      <td className="table-td text-xs text-gray-500 truncate" title={item.fields?.keterangan || ''}>{item.fields?.keterangan || '-'}</td>
    </tr>
  )
}
...
function ApprovedCard({ j, isAdmin, busy, pihakLabel, onUnapprove }: {
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="overflow-x-auto">
        {/* `table-fixed` + colgroup TETAP — lihat catatan panjang di
            Pengadaan.tsx (`PendingCard`/`ApprovedCard`). */}
        <table className="w-full table-fixed">
          <ColgroupBarang sesudah={[{ key: 'foto', berat: 5 }, { key: 'komptabel', berat: 6 }, { key: 'keterangan', berat: 10 }]} />
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              {/* Kembar dgn kartu draft di atas — ubah satu, samakan yang lain.
                  Sumbernya `l.fields`, yang sudah memuat SELURUH ASET_FIELD_COLS
                  hasil join ke `aset` (lihat loader), jadi tak ada query baru. */}
              <KolomBarangHead />
              <th className="table-th text-center">Foto</th>
            ...
            {j.lines.map(l => {
              ...
              return (
                <tr key={l.aset_id}>
                  ...
                  <td className="table-td text-xs text-gray-500 truncate" title={l.fields?.keterangan || ''}>{l.fields?.keterangan || '-'}</td>
                </tr>
              )
            })}
          </tbody>
...
function DokumenForm({ kategori, skpdId, skpdNama, judul, pihakLabel, cekNomorDipakai, onCancel, onSaved }: {
  ...
  async function simpan() {
    ...
    setSaving(false)
    if (error) { await konfirmasiGagal(konfirmasi, `Gagal menyimpan dokumen: ${error.message}`); return }
    onSaved()
  }
