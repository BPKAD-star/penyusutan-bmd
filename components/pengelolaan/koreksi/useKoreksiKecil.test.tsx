  ...
  createClient: () => ({
    from: () => {
      ...
      Object.assign(q, {
        ...
        single: async () => ({ data: asetRow, error: null }),
      })
      return q
    },
  }),
...
beforeEach(() => { asetRows = []; asetRow = {} })
afterEach(cleanup)

describe('usePencatatanGanda — survivor', () => {
  it('kandidat PERTAMA jadi survivor', () => {
    const { result } = renderHook(() => usePencatatanGanda(1))
...
describe('useSpesifikasi — centang & popup', () => {
  it('preset langsung tercentang (pintasan dari kartu Pemecahan)', () => {
    const { result } = renderHook(() => useSpesifikasi({ barang: br({ id: 'b1' }) }))
    expect(result.current.list.map(b => b.id)).toEqual(['b1'])
  })

  it('tanpa preset mulai kosong', () => {
    const { result } = renderHook(() => useSpesifikasi(null))
