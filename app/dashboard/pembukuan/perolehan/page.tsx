'use client'
// Pembukuan → Cara Perolehan (PLAN §5A)
// 4 jenis perolehan, semuanya import template Excel export e-bmd:
// SKPD tinggal export dari e-bmd lalu upload — tanpa format baru.
...
const TABS = [
  ...
  { key: 'hibah_masuk', label: 'Hibah Masuk' },
  { key: 'hasil_inventarisasi', label: 'Hasil Inventarisasi' },
  { key: 'perolehan_lainnya', label: 'Perolehan Lainnya' },
] as const
...
export default function PerolehanPage() {
  ...
  const [tab, setTab] = useState<string>('pengadaan')
  ...
  async function handleFile(f: File) {
    ...
    try {
      ...
      // Cari baris header: baris yang memuat 'NIBAR'
      ...
      const cUraian = col('uraianbarang')
      const cSpes = col('spesifikasinamabarang', 'spesifikasi')
      const cMerek = col('merek', 'tipe')
      const cJumlah = col('jumlahbarang', 'jumlah')
      const cSatuan = col('satuanbarang', 'satuan')
      const cHarga = col('hargasatuan')
      const cNilai = col('totalnilaibarang', 'totalnilai')
      const cTgl = col('tglperolehan', 'tanggalperolehan')
      const cKontrak = col('bentukkontrak')
      const cPenyedia = col('namapenyedia')
      const cNomor = col('nomor')
      const cKet = col('keterangan')

      const parsed: RowImport[] = []
      for (const r of grid.slice(headerIdx + 1)) {
        const nibar = String(r[nibarCol] ?? '').trim()
        // Kode = 7 segmen di kolom-kolom sebelum NIBAR (export e-bmd), digabung titik.
        // Fallback: 1 kolom berisi '1|3|2|05|...' → split non-digit.
        ...
        if (!kode && !nibar) continue // baris kosong
        ...
        parsed.push({
          ...
          spesifikasi: str(cSpes),
          merek_tipe: str(cMerek),
          ...
          bentuk_kontrak: str(cKontrak),
          nama_penyedia: str(cPenyedia),
          nomor_bast: str(cNomor),
          keterangan: str(cKet),
          valid: true, masalah: [],
        })
      }
      ...
      // Validasi: kode harus ada di kodefikasi_bmd, NIBAR belum terdaftar
  ...
  async function handleImport() {
    ...
    for (let i = 0; i < valid.length; i += 100) {
      ...
      const { data: inserted, error } = await supabase.from('aset').insert(chunk.map(r => ({
        ...
        merek_tipe: r.merek_tipe || null,
        jumlah: r.jumlah,
        satuan: r.satuan || null,
        harga_satuan: r.harga_satuan || null,
        nilai_perolehan: r.nilai,
        tgl_perolehan: r.tgl_perolehan,
        skpd_id: targetSkpd,
        cara_perolehan: tab,
        keterangan: r.keterangan || null,
      }))).select('id,nibar')

      if (error) { setMsg(`Error saat insert aset: ${error.message}`); break }

      // 2. Transaksi perolehan di ledger
      ...
      const { error: trxError } = await supabase.from('transaksi_bmd').insert(chunk
        ...
        .map(r => ({
          aset_id: idByNibar.get(r.nibar),
          jenis: tab,
          periode: periodeDariTanggal(r.tgl_perolehan || new Date().toISOString()),
          tanggal: r.tgl_perolehan || new Date().toISOString().slice(0, 10),
          nilai: r.nilai,
          skpd_tujuan: targetSkpd,
          payload: {
            bentuk_kontrak: r.bentuk_kontrak || null,
            nama_penyedia: r.nama_penyedia || null,
            nomor_bast: r.nomor_bast || null,
            lokasi: r.keterangan || null,
            sumber_import: fileName,
          },
          keterangan: `Import ${TABS.find(t => t.key === tab)?.label} — ${fileName}`,
        })))
      if (trxError) { setMsg(`Error saat mencatat transaksi: ${trxError.message}`); break }
      sukses += (inserted || []).length
    }

    if (!msg) setMsg(`${sukses} aset berhasil diimport sebagai ${TABS.find(t => t.key === tab)?.label}.`)
    setRows([])
    setFileName('')
    setImporting(false)
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-1 mb-4 border-b border-gray-200">
        {TABS.map(t => (
          <button key={t.key}
            onClick={() => { setTab(t.key); setRows([]); setMsg('') }}
      ...
      {tab !== 'pengadaan' && (
        <div className="mb-4 p-3 bg-amber-50 text-amber-800 rounded-lg text-sm">
          Sementara pakai template dasar yang sama dengan Pengadaan — kolom kontrak/penyedia boleh kosong
          (sumber dokumen: BA hibah / BA inventarisasi / dokumen perolehan lain).
        </div>
      )}
                  ...
                  <tr key={i} className={r.valid ? '' : 'bg-red-50/50'}>
                    <td className="table-td text-xs">
                      {r.valid
                        ...
                        : <span className="text-red-500">{r.masalah.join(', ')}</span>}
                    </td>
