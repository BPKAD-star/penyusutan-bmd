...
// sama dengan BA Rekon, Perolehan, & Perpindahan: sekali cetak, sekali tanda
// tangan, dan tiap lembar tetap bisa diambil sendiri kalau diminta.
//
// ⚠️ ANGKANYA DIMUAT `muatLaporanReklas` — SAMA dengan tab "Format Permendagri"
// di menu Pelaporan. Dua jalur angka untuk lembar yang sama adalah cara paling
...
import { ingatanCetak, kunciTtdReklas } from '@/lib/ingatanCetak'
...
/**
 * ⚠️ TANPA `plt`, sengaja — sama alasannya dgn lembar Perpindahan.
 *
 ...
 * tugas — "Plt. Pengguna Barang" bukan sebutan yang ada. Menyediakan
...
export default function CetakReklasPermendagriPage() {
  const supabase = createClient()
  const [lap, setLap] = useState<IdReklas>('penambahan')
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        // sisi LAIN dari yang diminta, dan tak ada yang menandainya.
        const lapQ = (q.get('lap') || 'penambahan') as IdReklas
        const fq = FORMAT_REKLAS[lapQ] as FormatReklas | undefined
        if (!fq) {
          throw new Error(`Lembar "${q.get('lap')}" belum ada. Yang tersedia: `
            + Object.keys(FORMAT_REKLAS).join(', ') + '.')
        }
        setLap(lapQ)
  ...
  const items: ItemLaporan<BarisReklas>[] = rows
    ...
    // ⚠️ `kode` = `kodeUtama`, BUKAN `aset.kode` — lihat catatannya di
    // components/pelaporan/ReklasFormatPermendagri.tsx.
    .map(r => ({
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-6 print:bg-white print:py-0">
      ...
      {/* ⚠️ PERINGATAN, bukan error — lembarnya tetap terbit. Tapi kolom (15) &
          (16) untuk barang-barang itu dicetak titik-titik, dan tanpa keterangan
      ...
      {siap && !gagal && tanpaPeny > 0 && (
        <div className="max-w-[1600px] mx-auto mb-3 px-4 no-print">
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-3 py-2">
            ...
            <b>{periodePosisiReklas(periode)}</b> — kolom Akumulasi &amp; Nilai Buku-nya dicetak
      ...
      <div id="cetak-reklas-permendagri"
